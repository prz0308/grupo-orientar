...
const contenedor = document.querySelector(".infoseries-column");
const player = document.getElementById("player-infoseries");
...
function generarLista() {
  ...
  infoseries.forEach((item, i) => {
    contenedor.innerHTML += `
      <div class="infoseries-item" data-index="${i}">
        <img src="${imagenBoton}" class="infoseries-item-boton">
        <div class="titulo-infoseries">${item.titulo}</div>
      </div>
    `;
  });
...
function activarClicks() {
  document.querySelectorAll(".infoseries-item").forEach((el) => {
...
function cargarVideo(index) {
  ...
  setTimeout(() => {
    source.src = infoseries[index].video;
    player.load();
...
function marcarSeleccionado(i) {
  document
    .querySelectorAll(".infoseries-item")
    .forEach((el) => el.classList.remove("selected"));

  document
    .querySelector(`.infoseries-item[data-index="${i}"]`)
...
window.addEventListener("DOMContentLoaded", () => {
  infoseries.sort((a, b) => a.titulo.localeCompare(b.titulo));
  generarLista();
  cargarVideo(0);
